...
// 機器人會照著舊的答。所以這支從 JSON 產出，由 vite plugin 自動跑。
//
// ## 為什麼是 vite plugin 而不是 npm prebuild
//
// Vercel 用 Vite 預設建置指令，**不跑 `npm run build`**——建置日誌裡沒有
// `Running "build" command:`，也沒有 run-p／vue-tsc 的輸出。所以 npm 的
// prebuild／postbuild 生命週期在 production 永遠不會觸發（repo 既有的
// prerender 也是因此做成 plugin 的）。掛在 vite 上才到得了每一種建置。
//
// 用法：node scripts/gen-knowledge-md.mjs   或   vite.config.ts 的 knowledgeBase()
// 產物：public/knowledge-base.md（vite 建置時複製進 dist，對外 /knowledge-base.md）
...
export function generateKnowledgeMd() {
